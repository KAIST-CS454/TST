...
def fitness(seq, g):
    isVisit = [False for i in range(g.size)] # node visit flag
    fit = 0
    for s in seq:
    ...
    for i in range(len(seq) - 1):
        try:
            ...
            d[(seq[i], seq[i+1])] = 1
    #print (d.keys())
    fit -= sum(max(0, 1.05 ** (i-1) - 1) for i in d.values() if i > 1)
    return fit
...
class Pheromone:
    def __init__(self, n, initP, alpha):
        self.adjMatrix = [[initP for i in range(n)] for j in range(n)] # pheromones are initialized with initP value
        self.size = n;
        self.alpha = alpha# evaporation rate

    def update(self, antSet):
        newPhes = [[[0, 0] for i in range(self.size)] for j in range(self.size)]
...
def ACORun(antSize, generation, seqLength, initP, rho, alpha, EFGfile):
    ...
    # ant initialization
    antList = [None for i in range(antSize)]
